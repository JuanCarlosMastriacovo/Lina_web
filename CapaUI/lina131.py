...
def _rubro_delete_restriction_message(conn, artrcodi: str) -> str | None:
    ...
    for relation in LinaArtr.CHILD_RELATIONS:
        ...
        if has_related:
            ...
            return (
                f"No se puede eliminar el rubro {artrcodi}. "
    ...
    try:
        if action == "create":
            ...
            if not LinaArtr.row_got_parents(insert_data, conn=conn):
                return HTMLResponse(
                    content="No existen todos los registros padres requeridos para crear el rubro.",
                    status_code=409,
                )
            ok = LinaArtr.row_insert(insert_data, conn=conn)
        else:
            ok = LinaArtr.row_update(
                {RUBRO_KEY_FIELD: resultado['normalized_data']['artrcodi']},
                {RUBRO_LABEL_FIELD: resultado['normalized_data']['artrdesc']},
                conn=conn,
            )
        ...
        if getattr(e, "errno", None) == 1062:
            return HTMLResponse(
                content=f"El codigo {resultado['normalized_data']['artrcodi']} ya existe.",
                status_code=409,
            )
    ...
    try:
        ...
        if not ok:
            ...
            if restriction_message:
                return HTMLResponse(content=restriction_message, status_code=409)
            return HTMLResponse(
                content=f"No se pudo eliminar el rubro {artrcodi}. Puede tener registros relacionados o una restricción de integridad.",
                status_code=400,
            )
        ...
        if owns_conn:
            conn.rollback()
        return HTMLResponse(content=f"Error al eliminar el rubro {artrcodi}: {e}", status_code=400)
    finally:
        if owns_conn:
...
async def recode_rubro(
    ...
    try:
        updated = LinaArtr.update_pk(
            {RUBRO_KEY_FIELD: resultado['normalized_data']['artrcodi']},
            resultado['normalized_data']['new_code'],
            conn=conn,
        )
    except IntegrityError as e:
        if owns_conn:
    ...
    return JSONResponse({
        "ok": True,
        "new_code": resultado['normalized_data']['new_code'],
        "message": "Código cambiado correctamente.",
    })