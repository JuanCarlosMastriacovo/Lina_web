...
class Lina42(linabase):
    """Módulo de Entradas y Salidas de Efectivo (LINA42)."""
    pass
...
async def lina42_index(request: Request):
    Lina42.set_prog_code(PROG_CODE)
    user = Lina42.get_current_user(request)
    if not user:
        return RedirectResponse("/login")
    return Lina42.templates.TemplateResponse(
        "lina42/form.html",
        {"request": request},
    )
...
async def lina42_prov_info(provcodi: str = Query(default="")):
...
@router.post("/save")
async def lina42_save(request: Request):
    """
    Graba un movimiento de caja (entrada o salida de efectivo).
    Payload: {provcodi, concepto, importe}
      importe > 0 → cajadebe (salida/egreso)
      importe < 0 → cajahabe (entrada/ingreso), se guarda el valor absoluto
    """
    Lina42.set_prog_code(PROG_CODE)
    user = Lina42.get_current_user(request)
    if not user:
    ...
    cajadebe = importe if importe > 0 else 0.0
    cajahabe = abs(importe) if importe < 0 else 0.0
    ...
    try:
        ...
        cur.execute(
            ...
            (empr, provcodi, cajafech, concepto, cajadebe, cajahabe),
        )
        cur.close()
        conn.commit()
        tipo = "Salida" if importe > 0 else "Entrada"
        return JSONResponse({"ok": True, "message": f"{tipo} de $ {abs(importe):,.2f} grabada."})
