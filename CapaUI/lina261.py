...
class Lina261(linabase):
    """Listado Resumen de Cobranzas (LINA261)."""
    pass
...
def _get_filas(empr: str, fecha_desde, fecha_hasta: date, cliecodi: int) -> list:
    """
    Retorna lista de [codm, nro, fecha_str, cta_str, cliente, efec, banc, total].
    ...
    try:
        ...
        sql = (
            ...
            "       cohe.cliecodi, cohe.coheefec, cohe.cohebanc, cohe.cohetota, cohe.coheobse,"
            ...
            " ORDER BY cohe.cohefech ASC, cohe.cohenume ASC"
        )
    ...
    for r in rows_raw:
        ...
        filas.append([
            ...
            int(r.get("cohenume") or 0),
            fecha_str,
            str(int(r.get("cliecodi") or 0)).zfill(4),
            cliente,
            float(r.get("coheefec") or 0),
            float(r.get("cohebanc") or 0),
            float(r.get("cohetota") or 0),
        ])
    return filas
...
async def lina261_index(request: Request):
    Lina261.set_prog_code(PROG_CODE)
    user = Lina261.get_current_user(request)
    if not user:
        return RedirectResponse("/login")
    return Lina261.templates.TemplateResponse(
        "lina261/seleccion.html",
        {"request": request, "hoy": date.today().isoformat()},
    )


@router.get("/clie/info")
async def lina261_clie_info(cliecodi: str = Query(default="")):
...
async def lina261_pdf(
    ...
    empr_code, empr_name = Lina261.get_empr_info()
    empr = ctx_empr.get() or DEFAULT_EMPR_CODE
    fecha_desde, fecha_hasta = _parse_fechas(desde, hasta)

    filas       = _get_filas(empr, fecha_desde, fecha_hasta, cliecodi)
    total_efec  = sum(f[5] for f in filas)
    total_banc  = sum(f[6] for f in filas)
    total_impo  = sum(f[7] for f in filas)

    filas_pdf = [
        [f[0], str(f[1]).zfill(6), f[2], f[3], f[4],
         fmt_money(f[5]), fmt_money(f[6]), fmt_money(f[7])]
        for f in filas
    ]

    template = pdf_jinja_env.get_template("lina261/main.html")
    html_str = template.render(
        ...
        empr_name       = empr_name,
        usuario         = user,
        titulo          = "Listado Resumen de Cobranzas",
        subtitulo       = _build_subtitulo(fecha_desde, fecha_hasta, cliecodi),
        fecha           = date.today().strftime("%d/%m/%Y"),
        hora            = datetime.now().strftime("%H:%M"),
        filas           = filas_pdf,
        total_efec      = fmt_money(total_efec),
        total_banc      = fmt_money(total_banc),
    ...
    return Response(
        ...
        headers    = {"Content-Disposition": "inline; filename=resumen_cobranzas.pdf"},
    )


@router.get("/xlsx")
async def lina261_xlsx(
    request:  Request,
    desde:    str = Query(default=""),
    hasta:    str = Query(default=""),
    cliecodi: int = Query(default=0),
    ...
    ws = wb.active
    ws.title = "Resumen Cobranzas"

    ws.merge_cells("A1:H1")
    ws["A1"]      = "Listado Resumen de Cobranzas"
    ...
    headers = ["Cpbt", "Número", "Fecha", "Cta.", "Cliente", "Efectivo", "Banco", "Total"]
    ws.append(headers)
    for cell in ws[5]:
    ...
    for fila in filas:
        ...
        for col_idx in (6, 7, 8):
            ws.cell(row=ws.max_row, column=col_idx).number_format = CURRENCY_FORMAT

    total_row = DATA_START + len(filas)
    ws.append(["", "", "", "", "TOTAL", total_efec, total_banc, total_impo])
    ...
    return Response(
        ...
        headers    = {"Content-Disposition": "attachment; filename=resumen_cobranzas.xlsx"},
    )
...
_COLUMNAS_TXT = [
    ...
    txt_col("Fecha",    10, "C"),
    txt_col("Cta.",     5,  "R"),
    txt_col("Cliente",  32, "L"),
    txt_col("Efectivo", 12, "R"),
    txt_col("Banco",    12, "R"),
    txt_col("Total",    12, "R"),
]
...
async def lina261_txt(
    ...
    totales_txt = ["", "", "", "", "TOTAL",
                   fmt_money(total_efec), fmt_money(total_banc), fmt_money(total_impo)]
    ...
    return Response(
        ...
        headers    = {"Content-Disposition": "inline; filename=resumen_cobranzas.txt"},
    )
